...
공유한다. 화면에 어떻게 보여줄지는 각자 log 콜백으로 정한다.
    ...
    1. 쇼핑몰이 전부 연결돼 있는지 확인한다             (shopmine/connect.py)
       -> 끊긴 곳이 있으면 [연결안된 쇼핑몰 연결재시도]
    2. [배송중] 탭에서 송장수정모드를 켜고, 택배사가 '경동택배' / '직접'인
       주문만 필터로 골라 전부 체크한다                 (shopmine/upload.py)
    3. 체크한 주문만 엑셀로 내보낸다                    (shopmine/export.py)
    4. 공급사에서 송장번호 조회 -> 업로드용 CSV 생성    (orchestrator.py)
       + CJ온스타일은 실제 크롬으로 별도 조회          (cjonstyle_bridge.py)
       + 한 주문번호가 여러 행인 주문은 여기서 제외    (excel_io.py)
    5. CSV를 [발송정보일괄등록(수정용)]으로 [일괄등록]  (shopmine/upload.py)
       -> 그리드의 '송장번호(수정용)' 컬럼이 채워진다
    6. 그 컬럼이 채워진 행만 체크하고 [송장번호수정]    (shopmine/grid.py)
       -> 쇼핑몰까지 실제 반영
    7. 결과에 오류가 없으면 송장수정모드를 끈다

왜 1단계에서 연결부터 보나: 6단계 [송장번호수정]은 샵마인이 실제로 쇼핑몰에
접속해 송장을 등록한다. 연결이 끊긴 쇼핑몰의 주문은 그때 가서 실패하는데, 그
시점에는 이미 다른 주문들이 반영된 뒤라 되돌리기 번거롭다.

왜 2단계에서 택배사로 거르나: '경동택배'와 '직접(전달)'은 실제 택배사가 아니라
...
from .shopmine import connect, excel_io, export, grid, upload
...
class PipelineResult:
    ...
    def __init__(self):
        ...
        self.picked: int = 0            # 2단계에서 고른 주문 수 (보이는 화면 기준)
        ...
        # 목록) 묶음. 4단계 로그에 한 번 나오지만 로그가 길어 묻히기 쉬워서,
...
def ensure_malls_connected(result: PipelineResult, *, log=print) -> bool:
    """1단계: 쇼핑몰이 전부 연결돼 있는지 확인하고, 끊긴 곳은 다시 연결한다.

    끊긴 채로 진행하면 6단계 [송장번호수정]에서 그 쇼핑몰 주문만 조용히
    실패하므로, 확인하지 못하면 아무것도 하지 않고 멈춘다.
    """
    log(f"[1/{STEPS}] 쇼핑몰 연결 상태 확인")
    try:
...
def select_and_export(result: PipelineResult, *, tab="배송중", log=print) -> bool:
    """2~3단계: 고칠 주문만 체크하고 그 주문만 엑셀로 내보낸다."""
    ...
    log("")
    log(f"[2/{STEPS}] 배송중 탭에서 송장을 고칠 주문 고르기 "
    ...
    log("")
    log(f"[3/{STEPS}] 체크한 주문만 엑셀로 내보내기")
    try:
...
def lookup_tracking(result: PipelineResult, *, limit=None, headless=False,
    ...
    """4단계: 공급사에서 송장번호를 조회해 업로드용 CSV를 만든다."""
    log("")
    log(f"[4/{STEPS}] 공급사에서 송장번호 조회")
...
def apply_csv(csv_path, order_ids, *, max_apply=100, expected_filled=None,
    ...
    """5~7단계. 엑셀 내보내기/조회가 이미 끝난 상태에서 실행한다.
    ...
    if stop_before_apply:
        log("")
        log("5단계까지 완료했습니다. 화면에서 '송장번호(수정용)' 컬럼을 확인한 뒤")
        log("채워진 행을 체크하고 [송장번호수정] 버튼을 직접 눌러주세요.")
        return result

    log("")
    log(f"[6/{STEPS}] 송장이 채워진 행만 체크하고 [송장번호수정]")
    try:
...
def run_from_csv(csv_path, *, max_apply=100, stop_before_apply=False,
    ...
    """이미 만들어둔 CSV로 5~7단계만 (쇼핑몰 연결 확인은 이때도 한다)."""
    result = PipelineResult()
...
def summarize(result: PipelineResult) -> str:
    ...
    사이트, 취소/품절)은 4단계 로그에 이미 한 번 나오지만 그 위로 로그가 길게
