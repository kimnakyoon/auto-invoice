...
무엇을 문의하나 - 딱 '2일 지남'으로 기록된 건만이다 (사용자 기준):
  ...
  - 기준은 조회를 돌린 날의 결과 엑셀에 적힌 '지난 일수'다. 오늘 다시 계산하지
    ...
    뺀 값이라(order_date.days_since) 금요일 주문은 화요일에 2일이 되고, 토·일
    주문은 월요일 주문으로 쳐서 수요일에 2일이 된다.
...
recipient_name, headless)가 있는 사이트만 처리하고(지금은 롯데온·지마켓·SSG·네이버·GSSHOP·롯데아이몰·NS홈쇼핑·11번가·패션플러스), 없는
...
AlreadyInquired를 내면(지마켓·SSG·네이버·GSSHOP·롯데아이몰·NS홈쇼핑·11번가·패션플러스는 등록 전에 문의내역을 뒤져 사람이 직접 남긴
...
def _message_for(target: InquiryTarget) -> str:
    ...
    문구에 주문번호를 넣는 사이트(GSSHOP - 사용자가 늘 "주문번호 수령인 배송 언제
    시작하나요?"로 남긴다)는 inquiry_message(recipient_name, product_url)로 받는다.
    """
    make = getattr(get_adapter(target.product_url), "inquiry_message", None)
    if make is None:
        return f"{target.recipient_name} 배송 언제 시작하나요?"
    if len(inspect.signature(make).parameters) >= 2:
        return make(target.recipient_name, target.product_url)
    return make(target.recipient_name)
    ...
    with sync_playwright() as p, contextlib.ExitStack() as stack:
        ...
        for i, t in enumerate(items, start=1):
            ...
            _append_ledger({
                ...
                "message": message,
                "posted_at": datetime.now().isoformat(timespec="seconds"),
                "confirmation": done,   # 어댑터가 확인한 완료 문구 (롯데온·지마켓·SSG·네이버·GSSHOP·롯데아이몰·NS홈쇼핑·11번가·패션플러스: 문의내역의 접수 상태·문의번호)
            })
