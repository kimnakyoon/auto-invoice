...
# 있으면 취소/품절 주문으로 본다. '환불완료'는 취소/품절과 같은 뜻이다(2026-09-10
# 옥션 주문상태 '환불완료'가 취소로 안 잡혀 배송조회 오류 페이지 실패로 남았던
# 건). '환불' 한 단어로 잡지 않는 이유: 화면 전체 텍스트를 넘기는 공급사에서는
# "교환/환불 안내" 같은 안내문에도 걸려 파싱 실패가 전부 취소 의심으로 보인다.
CANCELLED_KEYWORDS = ("취소", "품절", "불가", "환불완료")
