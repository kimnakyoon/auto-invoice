"""롯데아이몰 자동 로그인만 따로 검증한다.

auth/lotteimall_state.json(저장된 로그인 세션)을 **쓰지 않고** 빈 브라우저로
시작하기 때문에, 반드시 로그인 경로를 타게 된다. .env의
LOTTEIMALL_ID/LOTTEIMALL_PW로 자동 로그인이 실제로 되는지, 추가 본인인증을
요구받지는 않는지 확인하는 용도다.

실행:
    python scripts/test_lotteimall_login.py

성공하면 새로 로그인한 세션을 auth/lotteimall_state.json에 저장한다. 실패하면
...
# test_lotteimall_adapter.py와 같은 주문 - 로그인이 끝난 뒤 실제 조회까지 되는지 본다.
...
def main() -> None:
    ...
    load_dotenv()
    if not os.environ.get("LOTTEIMALL_PW"):
        print("⚠️ .env에 LOTTEIMALL_PW가 비어 있습니다. 자동 로그인 대신 수동 로그인 창이 뜹니다.")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print("   새 로그인 세션을 auth/lotteimall_state.json에 저장했습니다.")
        except Exception as e:
            print(f"❌ 실패: {type(e).__name__}: {e}")
            print("   (기존 세션 파일은 건드리지 않았습니다)")
            # 어디서 어긋났는지 사람이 확인할 수 있도록 마지막 화면을 남긴다.
            shot = Path(__file__).resolve().parent.parent / "logs" / "lotteimall_login_실패화면.png"
